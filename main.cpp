                       ...
                       const PaStreamCallbackTimeInfo*, PaStreamCallbackFlags, void*)
{
    ...
    PaUtil_WriteRingBuffer(&micAudio.ringBuffer, in, n);
    return paContinue;
}
                       ...
                       const PaStreamCallbackTimeInfo*, PaStreamCallbackFlags, void*)
{
    ...
    PaUtil_WriteRingBuffer(&spkAudio.ringBuffer, in, n);
    return paContinue;
}
...
void updateDrift(DriftState& ds, double nomMic, double nomSpk, double& outMic, double& outSpk) {
    ...
    if (wallSec - ds.lastDriftLogAt >= (uint64_t)DRIFT_LOG_INTERVAL) {
        ...
        cout << "[Drift @ " << wallSec << "s] mic=" << (int)micPpm << "ppm spk=" << (int)spkPpm << "ppm\n";
    }
...
struct Resampler {
    ...
    void init(int inCh, uint32_t inRate) {
        ...
        ma_data_converter_config cfg = ma_data_converter_config_init(
            ma_format_f32, ma_format_f32, (ma_uint32)inCh, 1, inRate, TARGET_RATE);
        cfg.resampling.algorithm = ma_resample_algorithm_linear;
        cfg.resampling.linear.lpfOrder = 4;
        if (ma_data_converter_init(&cfg, NULL, &conv) != MA_SUCCESS) exit(1);
        inChannels = inCh; initialised = true;
    }

    vector<float> drain(SharedAudio& src, double inRate) {
        if (!initialised) init(inChannels, (uint32_t)round(inRate));
        ...
        ma_uint64 actualIn = available, actualOut = framesOut;
...
void processingThread() {
    ...
    micRes.inChannels = micChannels;
    spkRes.inChannels = spkChannels;
    ...
    auto drainAndQueue = [&]() {
        ...
        size_t frames = max(micMono.size(), spkMono.size());
        {
            ...
            for (size_t i = 0; i < frames; i++) {
                ...
                float mixed = (i < micMono.size() && i < spkMono.size()) ? (m + s) * 0.5f : (m + s);
...
void senderThread(const string& outPath) {
    ...
    while (true) {
        vector<float> chunk;
        {
            ...
            outQueue.cv.wait(lk, []{
                return outQueue.frames.size() >= SEND_CHUNK_FRAMES || (processingDone.load() && !outQueue.frames.empty()) || (processingDone.load() && outQueue.frames.empty());
            });
        ...
        for(size_t i=0; i<chunk.size(); i++) pcm[i] = (int16_t)(max(-1.0f, min(1.0f, chunk[i])) * 32767.0f);
...
int main() {
    ...
    for (int i = 0; i < numDevices; i++) {
        ...
        cout << "ID: " << i << " | " << d->name << " | In: " << d->maxInputChannels << " | Out: " << d->maxOutputChannels << "\n";
    }

    int micDev, spkDev;
    cout << "\nEnter MIC ID: "; cin >> micDev;
    cout << "Enter SPEAKER ID: "; cin >> spkDev;
    cin.ignore();
    ...
    micParams.device = micDev; micParams.channelCount = micChannels; micParams.sampleFormat = paFloat32; micParams.suggestedLatency = micInfo->defaultLowInputLatency;
    spkParams.device = spkDev; spkParams.channelCount = spkChannels; spkParams.sampleFormat = paFloat32; spkParams.suggestedLatency = spkInfo->defaultLowInputLatency;

    PaStream *micStream, *spkStream;
    Pa_OpenStream(&micStream, &micParams, NULL, micRate, FRAMES_PER_BUFFER, paClipOff, micCallback, NULL);
    Pa_OpenStream(&spkStream, &spkParams, NULL, spkRate, FRAMES_PER_BUFFER, paClipOff, spkCallback, NULL);
